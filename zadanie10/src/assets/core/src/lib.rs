...
#[wasm_bindgen]
pub fn eval_moves(_board: Vec<JsValue>, width: usize) -> Move {
    ...
    while iteration < 5 {
        ...
        if lines.len() > 0 {
            for line in lines {
                let moves = get_moves(line, &board);
                for m in moves {
    ...
    for (_k, v) in move_map {
        if best_move.is_none() {
            best_move = Some(v);
        } else {
            let b = best_move.unwrap();
            if v.score > b.score {
                best_move = Some(v);
            }
        }
...
fn get_moves(line: Vec<Block>, _board: &Vec<Vec<i8>>) -> Vec<Move> {
    let own = line[0].value == 2;
